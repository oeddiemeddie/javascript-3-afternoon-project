...
function employeeUpdater(){
  for(i = 0; i < employees.length; i++) { 
    if(employees[i].firstName === 'Theo') {
      employees.splice(i, 1)  // "i" is your current position
    } 
...
function removeDuplicates() { //WRONG
  for (let i = 0; i < workplaceAccidents.length; i++) {
    for (let a = i + 1; a < workplaceAccidents.length; a++) {
      if (workplaceAccidents[i] === workplaceAccidents[a]) {
...
//Code Here
var grumpyActivity = cat.catFriends[0].activities[1]; //cat, catFriends, activities, 1st index of the array
//catFriends(activities[1])
